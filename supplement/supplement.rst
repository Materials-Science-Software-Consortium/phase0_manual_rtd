...
Perlスクリプトanimate.plは、mode.dataに出力されている振動モードの固有ベクトルのデータを読み込み、基準振動の軌跡を拡張trajectory形式ファイルに変換します。
...
| ekina  | 系の運動エネルギー                                         |
+--------+------------------------------------------------------------+
| econst | 系の保存量、すなわちエネルギー一定の分子動力学\            |
|        | シミュレーションの場合系の全エネルギー、\                  |
|        | 温度一定の分子動力学シミュレーションの場合\                |
|        | 系の全エネルギーに熱浴のエネルギーを加えた量です。         |
+--------+------------------------------------------------------------+
...
stressmx      ストレステンソルの最大値（収束判定対象値）
============= ========================

なお、iter_ionは、格子更新の度に1にリセットされます。

.. _output_nfdynm_section:
...
+-----+---------------------------------------------------------------+
| (b) | ntyp = の後には使用されている原子種の数が記述されています。   |
|     | この例では2です。                                             |
|     | また、natm = の後には原子数が書かれています。                 |
|     | この例では9 です。                                            |
+-----+---------------------------------------------------------------+
| (c) | (natom->type) の後は、原子と原子種のマッピングです。          |
|     | この例では、1番目から6 番目の原子の原子種は1、                |
|     | 7 番目から9 番目の原子種は2という元素に対応します。           |
+-----+---------------------------------------------------------------+
| (d) | (speciesname) の後は、原子種と元素のマッピングです。          |
|     | この例では、原子種1はO（酸素）、原子種2はSi（シリコン）です。 |
+-----+---------------------------------------------------------------+
| (e) | 各ステップでの情報が記述されています。この例では、\           |
|     | イオンの更新回数が1 回目、SCF の更新回数が24 回となります。   |
+-----+---------------------------------------------------------------+
| (f) | 原子位置とその原子に働いている力が出力されます。              |
|     | 1列目は原子のID、2から4列目が原子位置のx,y,z座標、            |
|     | 5から7 列目が原子に作用する力のx,y,z成分です。                |
|     |                                                               |
|     | 分子動力学計算では（printoutlevelブロックでvelocity変数に\    |
|     | 2を設定した場合）、5から7 列目に原子の速度が出力され、\       |
|     | 原子に作用する力は、8から10列目に\                            |
|     | x, y, z成分がそれぞれ出力されいます。                         |
+-----+---------------------------------------------------------------+
