...
PHASEの擬ポテンシャルは大きく分けてフローズンコア型とPAW型 [Blöchl94a]_ の２種類があります。
...
.. .. _section_spin_axis_parallelization:

.. スピン軸に対する並列計算 (2024.01以降)
.. ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

.. スピンを考慮した計算を行う場合、スピン軸の並列を行うことができます。すなわち、アップスピン状態とダウンスピン状態をそれぞれ異なるプロセスで処理させることができます。二次元版の場合、典型的には以下のようなコマンドになります。

.. .. parsed-literal::

..  % mpirun -np NP PHASE_INSTALL_DIR/|PHASE020XX.YY|/bin/phase ne=NE nk=NK ns=NS

.. ``NS`` として許されるのは1か2のみです。また、ノンコリニア計算の場合は利用できません。
...
ファイルnfefn.data（またはfile_names.dataファイルにおいてF_ENF によって指定されるファイル）には、系の全エネルギーと原子に働く力の最大値が記述されます。

格子の緩和や分子動力学シミュレーションを行った場合は、情報が追加されます。
それぞれ計算機能の説明を参照してください。

基本的な構造緩和を行った後のnfefn.dataの例を示します。
...
.. table::
 ...
 +------------+--------------------------------------------------------+
 | iter_ion   | イオン位置の更新回数です。                             |
 +------------+--------------------------------------------------------+
 | iter_total | SCF ループの更新回数です。                             |
 |            | この数字は通算の値が記述されます。                     |
 |            | （イオン位置の更新でリセットされません。）             |
 +------------+--------------------------------------------------------+
 | etotal     | 全エネルギーです。（単位：Hartree）                    |
 +------------+--------------------------------------------------------+
 | forcmx     | 原子に働く力の最大値です。(単位：Hartree / Bohr)       |
 |            |                                                        |
 |            | mobile属性がonの原子のみが評価の対象です。             |
 |            | この値が入力ファイルで指定した構造緩和の収束判定条件を\|
 |            | 満たすまで、計算は繰り返し実行されます。               |
 +------------+--------------------------------------------------------+
...
PHASE-Viewerを用いると、原子配置を可視化できます。
また、座標データ変換ツール ``conv.py`` （:numref:`commands_and_tools_convpy` ）を利用して、CIF等のフォーマットに変換できます。
変換後のCIFファイルをVESTA等のソフトウェアで可視化して、計算結果の確認に利用します。

書式の詳細は
:numref:`output_nfdynm_section` 
を参照してください。
...
.. [Blöchl94a] P\. E. Blöchl,  Phys. Rev. B 50 vol. 24 17953–17978 (1994).
